  ...
  return (
    <div className="app-frame">
      <aside className={`sidebar ${menuOpen ? 'sidebar--open' : ''}`}>
        <div className="brand-block"><div className="brand-mark" aria-hidden="true"><span>SF</span></div><div><strong>SafeFleet AI</strong><span>Fleet Safety Platform</span></div></div>
        <div className="workspace-pill"><span className="workspace-dot" /><div><strong>{session.organization.name}</strong><small>{session.user.role} · {session.organization.slug}</small></div></div>
        <nav className="primary-nav" aria-label="Primary navigation">
          {navigation.map((item) => <NavLink key={item.to} to={item.to} end={item.to === '/'} onClick={() => setMenuOpen(false)} className={({ isActive }) => `nav-item ${isActive ? 'nav-item--active' : ''}`}><span className="nav-monogram" aria-hidden="true">{item.monogram}</span><span className="nav-label">{item.label}</span><span className="phase-tag">{item.phase}</span></NavLink>)}
        </nav>
        <div className="sidebar-footer"><span className="eyebrow">Signed in</span><strong>{session.user.fullName}</strong><small>{session.user.email}</small><button type="button" className="sidebar-logout" onClick={() => auth.logout()}>Sign out</button></div>
      </aside>
